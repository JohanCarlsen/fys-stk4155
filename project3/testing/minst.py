...
# fig, axes = plt.subplots(2, n_show, figsize=set_size())

# for i, im in enumerate(minst.images[rand_inds]):
#     ax = axes.flatten()[i]
#     ax.set_title(f'Label: {y[rand_inds[i]]}')
...
for i in range(len(ypred)):
    pred = np.argwhere(ypred[i, :] == 1)[0][0]
    true = np.argwhere(y_test[i, :] == 1)[0][0]
    
    # print(f'\nPredicted label: {pred}')
    # print(f'Correct label: {true}')

print(f'Accuracy: {np.average(ypred == y_test)}')
